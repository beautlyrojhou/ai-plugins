...
description: Tech lead's framework for architecting solutions inside a team's domain while staying coherent with Bitwarden's holistic architecture. Covers security mindset, blast radius, architectural judgment, Bitwarden-specific constraints, working with the architecture group, and working with initiative shepherds. Use when planning a solution, reviewing architecture within your team's scope, assessing blast radius, evaluating trade-offs, or deciding whether a choice needs architecture-group input.
---
...
Your team has autonomy over decisions inside its domain. Architecture doesn't gate-keep team-level work. What Architecture does is maintain the holistic view — the portfolio of cross-cutting initiatives, the patterns that span teams, the decisions that will be expensive to change later. Your job is to recognize when a choice you're about to make has implications that benefit from that wider view, and pull Architecture in before — not after — you ship.
...
- **Structural decisions costly to change later.** Data model choices, service boundaries, protocol selection, auth integration — decisions whose cost compounds if you get them wrong.
...
- **New precedent.** You're about to do something Bitwarden hasn't done before in a way that will likely be repeated by others.
- **External-facing output.** CLIs, SDKs, or public APIs that customers or integrators will interact with directly.

If none of those apply, decide inside your team and move. If any of them apply, surface it — through your EM into the monthly Architecture/Platform sync, by attending Architecture Council, or by filing a Technical Strategy Idea (see `Skill(contributing-to-technical-strategy)`).
...
When your team is receiving an initiative epic, the shepherd is your counterpart. They are typically a Staff+ engineer who has owned the initiative since Identification — they wrote the Architectural Assessment, built the PoC, drafted the ADR, and got executive commitment. What they do not do is write your team's stories or run your team's implementation.
...
- **You own:** your team's story breakdown, acceptance criteria, sizing, implementation sequencing, and your team's PRs.

Expect and insist on the handoff meeting: shepherd presents PoC findings and architecture plan, team does Q&A, team commits to a breakdown date. After that, your team does the breakdown — not the shepherd. The shepherd is available for approach questions, reviews 1–2 early PRs from your team for alignment with the PoC pattern, and surfaces cross-team dependencies. Everything else is yours.

Two failure modes to avoid:

- **The shepherd writes your stories.** Stories your team didn't write are stories your team won't own. Insist on a handoff meeting and a team breakdown session.
- **You drift from the PoC pattern without flagging it.** Drift across teams is exactly what the shepherd is there to prevent. If you find yourself deviating, tell the shepherd before you ship, not after.
...
- Accepting an initiative epic without capacity explicitly allocated by your EM and engineering leadership
